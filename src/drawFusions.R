...
--transcriptSelection=<character> transcript selection [default: coverage].\n
...
--color1=<character> color 1 used in the visualization [default: #a7c4e5].\n
...
opt$squishIntrons <- as.logical(opt$squishIntrons)
opt$render3dEffect <- as.logical(opt$render3dEffect)
opt$showVicinity <- as.numeric(opt$showVicinity)
...
# 33 i 42 intron etc.
fusion <- read.table(opt$inputFile, sep = "\t", row.names = NULL)

direction1 <- ifelse(fusion$V1 > fusion$V4, "downstream",
                     ifelse((fusion$V1 == fusion$V4 & fusion$V2 > fusion$V5), "downstream", "upstream"))

direction2 <- ifelse(direction1 == "downstream", "upstream", "downstream")

fusionTsv <- data.frame(`gene1` = fusion$V28,
                                    `gene2` = fusion$V37,
                                    `strand1(gene/fusion)` = paste0(fusion$V9, "/", fusion$V9),
                                    `strand2(gene/fusion)` = paste0(fusion$V10, "/", fusion$V10),
                                    `breakpoint1` = paste0(fusion$V1, ":", fusion$V2),
                                    `breakpoint2` = paste0(fusion$V4, ":", fusion$V5),
                                    `site1` = 'splice-site',
                                    `site2` = 'splice-site',
                                    `type` = fusion$V12,
                                    `direction1` = direction1,
                                    `direction2` = direction2,
                                    transcript_id1 = fusion$V30,
                                    transcript_id1 = fusion$V39,
                                    fusion_flag = fusion$V46,
                        total_region_count1 = fusion$V35,
                        total_region_count2 = fusion$V44,
                        assembly_score = fusion$V14,
...
fusions <- fusionTsv
if (colnames(fusions)[1] == "gene1") { # Arriba output
  colnames(fusions)[colnames(fusions) %in% c("gene1", "strand1.gene.fusion.", "strand2.gene.fusion.")] <- c("gene1", "strand1", "strand2")
...
pdf(opt$outputFile, onefile=T, width=as.numeric(opt$pdfWidth), height=as.numeric(opt$pdfHeight), title=opt$fusionsFile)
if (nrow(fusions) == 0) {
...
exons <- read.table(opt$exonsFile, header=F, sep="\t", comment.char="#", quote="", stringsAsFactors=F)[,c(1, 3, 4, 5, 7, 9)]
...
if (is.null(opt$proteinDomainsFile)) {
  ...
  proteinDomains <- read.table(opt$proteinDomainsFile, header=F, sep="\t", comment.char="", quote="", stringsAsFactors=F)[,c(1,4,5,7,9)]
...
if (!as.logical(opt$render3dEffect)) # nullify function, if no 3D effect should be drawn
  rawVerticalGradient <- function(left, right, y, color, selection=NULL) { }
...
drawProteinDomains <- function(fusion, exons1, exons2, proteinDomains, color1, color2, mergeDomainsOverlappingBy, optimizeDomainColors) {
  ...
  text(0.5, titleY, ifelse(fusion$reading_frame %in% c("in-frame", "out-of-frame"), paste(fusion$reading_frame, "fusion"), ifelse(fusion$reading_frame == "stop-codon", "stop codon before fusion junction", "reading frame unclear")), adj=c(0.5, 1), cex=opt$fontSize)
...
for (fusion in 1:nrow(fusions)) {
  ...
  if (opt$alignmentsFile != "") {
    ...
    determineCoverageRange <- function(exons, gene, contig, breakpoint, showVicinity) {
      ...
      return(IRanges(min(closestExons$start, breakpoint-opt$showVicinity), max(closestExons$end, breakpoint+opt$showVicinity)))
    }
    coverageRange1 <- determineCoverageRange(exons, fusions[fusion,"gene1"], fusions[fusion,"contig1"], fusions[fusion,"breakpoint1"], opt$showVicinity)
    coverageRange2 <- determineCoverageRange(exons, fusions[fusion,"gene2"], fusions[fusion,"contig2"], fusions[fusion,"breakpoint2"], opt$showVicinity)
    ...
    # get coverage track
    coverage1 <- readCoverage(opt$alignmentsFile, fusions[fusion,"contig1"], coverageRange1)
    coverage2 <- readCoverage(opt$alignmentsFile, fusions[fusion,"contig2"], coverageRange2)
  }
  ...
  if (opt$showVicinity > 0) {
    if (fusions[fusion,"site1"] == "intergenic")
      for (gene in unique(exons[exons$contig == fusions[fusion,"contig1"] & exons$exonNumber != "intergenic" &
                                (between(exons$end, fusions[fusion,"breakpoint1"]-opt$showVicinity, fusions[fusion,"breakpoint1"]+opt$showVicinity) |
                                 between(exons$start, fusions[fusion,"breakpoint1"]-opt$showVicinity, fusions[fusion,"breakpoint1"]+opt$showVicinity)),"geneName"]))
      ...
      if (fusions[fusion,"site2"] == "intergenic")
        for (gene in unique(exons[exons$contig == fusions[fusion,"contig2"] & exons$exonNumber != "intergenic" &
                                  (between(exons$end, fusions[fusion,"breakpoint2"]-opt$showVicinity, fusions[fusion,"breakpoint2"]+opt$showVicinity) |
                                   between(exons$start, fusions[fusion,"breakpoint2"]-opt$showVicinity, fusions[fusion,"breakpoint2"]+opt$showVicinity)),"geneName"]))
  ...
  if (opt$alignmentsFile != "") {
    coverageNormalization <- ifelse(
      opt$squishIntrons, # => ignore intronic coverage
      max(
  ...
  yIdeograms <- ifelse(opt$alignmentsFile != "", 0.94, 0.84)
  yBreakpointLabels <- ifelse(opt$alignmentsFile != "", 0.86, 0.76)
  yCoverage <- 0.72
  yExons <- 0.67
  yGeneNames <- 0.58
  yFusion <- 0.5
  ...
  if (opt$alignmentsFile != "") {
    ...
    text(-0.025, yCoverage, "0", adj=c(1,0.5), cex=0.9*opt$fontSize)
    text(-0.025, yCoverage+0.1, coverageNormalization, adj=c(1,0.5), cex=0.9*opt$fontSize)
    text(-0.05, yCoverage+0.08, "Coverage", srt=90, cex=0.9*opt$fontSize, adj=c(1,0.5))
  ...
  if (opt$squishIntrons) {
    for (exon in 1:nrow(exons1))
      if (exons1[exon,"type"] != "CDS") # don't draw coverage twice for coding regions
        drawCoverage(exons1[exon,"left"], exons1[exon,"right"], yCoverage, coverage1, exons1[exon,"start"], exons1[exon,"end"], opt$color1)
  } else {
    drawCoverage(min(exons1$left), max(exons1$right), yCoverage, coverage1, min(exons1$start), max(exons1$end), opt$color1)
  }
  ...
  if (opt$squishIntrons) {
    for (exon in 1:nrow(exons2))
      if (exons2[exon,"type"] != "CDS") # don't draw coverage twice for coding regions
        drawCoverage(gene2Offset+exons2[exon,"left"], gene2Offset+exons2[exon,"right"], yCoverage, coverage2, exons2[exon,"start"], exons2[exon,"end"], opt$color2)
  } else {
    drawCoverage(gene2Offset+min(exons2$left), gene2Offset+max(exons2$right), yCoverage, coverage2, min(exons2$start), max(exons2$end), opt$color2)
  }
  ...
  if (!is.null(fusions[fusion,"fusion_transcript"])) {
    ...
    # transcript 1
    text(fusionOffset2, yTranscript, bquote(.(fusion_transcript1) * phantom(.(non_template_bases1))), col=darkColor1, adj=c(1,0.5), cex=opt$fontSize)
    # transcript 2
    text(fusionOffset2, yTranscript, bquote(phantom(.(non_template_bases2)) * .(fusion_transcript2)), col=darkColor2, adj=c(0,0.5), cex=opt$fontSize)
    # non-template bases
    text(fusionOffset2, yTranscript, non_template_bases1, adj=c(1,0.5), cex=opt$fontSize)
    text(fusionOffset2, yTranscript, non_template_bases2, adj=c(0,0.5), cex=opt$fontSize)
  }
